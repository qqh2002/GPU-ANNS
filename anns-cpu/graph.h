...
private:
    ...
    void rank_and_switch_ordered(idx_t v_id,idx_t u_id){
        ...
        auto offset = ((int)v_id) << vertex_offset_shift;
        ...
        for(int i = offset + edges[offset] - 1;i > offset;--i){
    ...
    void compute_distance_naive(int offset,std::vector<dist_t>& dists){
    ...
    void compute_distance(int offset,std::vector<dist_t>& dists){
...
public:
    ...
    FixedDegreeGraph(Data* data) : data(data){
        ...
        edges = std::vector<idx_t>(((int)num_vertices) << vertex_offset_shift);
        edge_dist = std::vector<dist_t>(((int)num_vertices) << vertex_offset_shift);
    }
    ...
    void add_vertex(idx_t vertex_id,std::vector<std::pair<int,value_t>>& point){
        ...
        auto offset = ((int)vertex_id) << vertex_offset_shift;
    ...
    void astar_multi_start_search(const std::vector<std::pair<int,value_t>>& query,int k,std::vector<idx_t>& result){
        ...
        for(int iter = 0;iter < max_step && !q.empty();++iter){
            ...
            if(topk.size() > k)
                topk.pop();
            auto offset = ((int)now.second) << vertex_offset_shift;
            auto degree = edges[offset];
            for(int i = 0;i < degree;++i){
    ...
    void print_stat(){
        ...
        int sum = 0;
        std::vector<int> histogram(2 * degree + 1,0);
        for(int i = 0;i < n;++i){
    ...
    void print_edges(int x){
        for(int i = 0;i < x;++i){
            int offset = i << vertex_offset_shift;
    ...
    void dump(std::string file = "bfsg.graph"){
        ...
        auto num_vertices = data->max_vertices();
        fwrite(&edges[0],sizeof(edges[0]) * (((int)num_vertices) << vertex_offset_shift),1,fp);
        fclose(fp);
    }
    ...
    void load(std::string file = "bfsg.graph"){
        ...
        auto num_vertices = data->max_vertices();
        auto cnt = fread(&edges[0],sizeof(edges[0]) * (((int)num_vertices) << vertex_offset_shift),1,fp);
        fclose(fp);
    }
